    ...
    return (
        <section className="py-24 bg-white/40 border-y border-white/60 backdrop-blur-sm relative z-10" id="testimonials">
            ...
            <div className="relative group w-full">
                <div className="absolute inset-y-0 left-0 w-8 md:w-32 bg-gradient-to-r from-[#fbfbfd] to-transparent z-10 pointer-events-none" />
                <div className="absolute inset-y-0 right-0 w-8 md:w-32 bg-gradient-to-l from-[#fbfbfd] to-transparent z-10 pointer-events-none" />
                
                <div className="flex overflow-x-auto gap-6 px-12 md:px-24 pb-8 pt-4 snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] w-full">
                    {testimonials.map((item, index) => (
                        <div 
                            key={index} 
                            className="glass-panel px-8 py-6 w-[85vw] sm:w-[320px] max-w-[400px] flex flex-col justify-between whitespace-normal shrink-0 snap-center transition-transform duration-300 hover:-translate-y-2"
                        >
