...
import { Sparkles, ChevronLeft, Check, ChevronDown, ChevronUp, Music, Play, Volume1, Volume2, Circle, CircleDot, Activity, Search, Bold, Italic, Link, AtSign, Hash, Home, PlusSquare, MessageCircle, User, Heart, Share2, VolumeX, X, Send, Clock, Bell, Plus } from 'lucide-react';
    ...
    <section className="container mx-auto px-6 py-24 flex flex-col lg:flex-row-reverse items-center justify-center gap-16 lg:gap-24 overflow-hidden scroll-mt-10" id="feed" ref={containerRef}>
      ...
      <div className="flex-1 w-full max-w-md mx-auto relative z-10 flex justify-center">
        ...
        >
          ...
          {step > 0 && (
            <div className="absolute bottom-0 inset-x-0 bg-[#0c0c10]/95 backdrop-blur-xl border-t-[0.5px] border-[rgba(255,255,255,0.07)] flex items-end justify-around px-2 z-50 pt-[8px] pb-[10px]">
              <button onClick={() => setStep(4)} className={`flex flex-col items-center justify-center w-12 cursor-pointer transition-colors ${step === 4 ? 'text-white' : 'text-white/40 hover:text-white'}`}>
                <Home size={22} strokeWidth={step === 4 ? 2.5 : 2} />
                <span className={`text-[7px] font-medium mt-[3px] ${step === 4 ? 'text-white' : 'text-white/30'}`}>Home</span>
                {step === 4 && <div className="w-[4px] h-[4px] bg-[#d4f56a] rounded-full mt-[3px]" />}
              </button>
              <button onClick={() => setStep(6)} className={`flex flex-col items-center justify-center w-12 cursor-pointer transition-colors ${step === 6 ? 'text-white' : 'text-white/40 hover:text-white'}`}>
                <Search size={22} strokeWidth={step === 6 ? 2.5 : 2} />
                <span className={`text-[7px] font-medium mt-[3px] ${step === 6 ? 'text-white' : 'text-white/30'}`}>Search</span>
                {step === 6 && <div className="w-[4px] h-[4px] bg-[#d4f56a] rounded-full mt-[3px]" />}
              </button>
              <button onClick={() => setStep(1)} className="flex flex-col items-center justify-center w-12 cursor-pointer transition-transform active:scale-95 mb-1">
                <div className="w-[34px] h-[34px] bg-white rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                  <Plus size={20} className="text-[#0c0c10]" strokeWidth={2.5} />
                </div>
              </button>
              <button className="flex flex-col items-center justify-center w-12 cursor-pointer transition-colors text-white/40 hover:text-white">
                <MessageCircle size={22} strokeWidth={2} />
                <span className="text-[7px] font-medium mt-[3px] text-white/30">Chat</span>
              </button>
              <button onClick={() => setStep(5)} className={`flex flex-col items-center justify-center w-12 cursor-pointer transition-colors ${step === 5 ? 'text-white' : 'text-white/40 hover:text-white'}`}>
                <User size={22} strokeWidth={step === 5 ? 2.5 : 2} />
                <span className={`text-[7px] font-medium mt-[3px] ${step === 5 ? 'text-white' : 'text-white/30'}`}>Profile</span>
                {step === 5 && <div className="w-[4px] h-[4px] bg-[#d4f56a] rounded-full mt-[3px]" />}
              </button>
            </div>
          )}
        </motion.div>
